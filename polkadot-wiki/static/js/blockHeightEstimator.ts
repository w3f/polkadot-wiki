...
const driver = async (dateOrHours: string) => {
...
driver(userInput)
  .then((res) => {
    if (userInput.includes('/')) {
      ...
      console.log(`In ${userInput} ${parseInt(userInput) == 1 ? "hour" : "hours"}, the chain will be around block height #${res}`);
    }
    return res;
  })
  .catch(console.error)
