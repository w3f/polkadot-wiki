...
high level, shared security means that all parachains which have connected to the Polkadot relay
chain by leasing a parachain slot will benefit from the economic security provided by the relay
chain validators.
...
of security. For example, one suggestion to scale blockchains is that of _scale by altcoins_ which
...
becoming a larger industry which increasingly centralizes on key players, it is becoming more real
that a single actor may control enough hash power to attack a chain. This means that smaller chains
which cannot maintain a secure amount of hash power on their networks could potentially be attacked
...
Security is independent of the number of parachains which are connected to the Polkadot relay chain.
...
of DOTs which are bonded and staked. The greater the number of DOTs staked by honest validators and
...
consensus, like the one which enables fast payments on
