...
accounts**. For more details about proxy accounts, visit this wiki's [dedicated page](../../knowledge-base/proxies.md).
...
!!!info "Staking Migration to Asset Hub"
    **Staking proxies** are automatically migrated to Asset Hub along with other proxy types (except the "Any" type). However, **proxy announcements are not migrated** and will be deleted from the Relay Chain.
...
In Polkadot's [Nominated Proof-of-Stake (NPoS)](../../reference/consensus.md#nominated-proof-of-stake), nomination intents are placed in a semi-sorted list called the bags list. The [Bags-List pallet](https://paritytech.github.io/polkadot-sdk/master/pallet_bags_list/index.html) in the Polkadot SDK is
...
Simple payouts require one transaction per validator, per [era](../../knowledge-base/glossary.md#era), to
