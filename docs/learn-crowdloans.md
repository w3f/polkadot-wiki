...
crowdloan is live, the parachain configuration will be locked and it will be what is deployed as the
...
- The campaign creation form requires setting a crowdloan cap &mdash; the maximum amount a campaign
  can collect. A team can still win an auction if the cap has not been reached.
...
- "First slot" field refers to the first slot you want to bid for. If the current auction
  encompasses slots `(3, 4, 5, 6)`, your first slot can be at least `3`. The last slot must also be
  within that range.
