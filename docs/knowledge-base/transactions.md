...
pair that does not control funds. For example, validator's [session keys](../../reference/cryptography.md)
...
[good practice not to blind sign transactions](../knowledge-base/transaction-attacks.md) to avoid being
victim of an attack.
...
[for security reasons](transaction-attacks.md#replay-attack), it is highly recommended
...
For more detailed information about the arguments accepted by each of these calls, see [the Metadata Explorer](metadata.md). The emitted events for each call are summarized below:
...
[a multitude of possible attacks](../knowledge-base/transaction-attacks.md) that will prevent you to send
