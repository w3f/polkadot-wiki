...
- **Reserved Balance** (also called holds, or held balance) is the balance removed from free and does not overlay. It can be slashed, but only after all the free balance has been slashed. Reserved balance is used for:
    ...
    - deposits such as [governance](./learn-polkadot-opengov.md) decision and submission deposits, [identity](./learn-identity.md) deposits, and [proxy](../../knowledge-base/proxies.md) and [multi-signature](../../knowledge-base/multisig.md) accounts deposits. It cannot be used for transfers or paying fees.
