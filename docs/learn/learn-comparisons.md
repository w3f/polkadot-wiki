...
through a network of **sequencers**. In theory the role of sequencing transactions and publishing
them could be assumed by separate entities. In practice today they are the same machine, but this is
likely to change as rollups decentralize more. This mechanism could include thousands of
...
valid. If a dispute arises, a challenge mechanism is triggered, requiring participants to provide
...
| **Latency**        | Time to finality depends on challenge periods. Optimistic rollups often take hours or even days for finality. Polkadot rollups reduce this to around 30 minutes. | Near-instant finality in theory. In practice, systems like zkSync can take up to a day to finalize, often due to artificial safeguards rather than inherent technical limitations. |
...
| **State Transition Logic**   | General-purpose but constrained by zk-circuit implementation complexity.                                                                                                                                                                                       | Can support state transitions beyond EVM compatibility by interpreting other virtual machine (VM) logic within the EVM environment<sup>1</sup>.                                          | Each parachain can define its unique state transition function (STF), compiled to Wasm and validated per Parachain Protocol rules.                                                                                                                                                                                                                  |
...
| **Data Availability**        | Data availability requirements posted by the optimistic and ZK rollups is precisely the same.                                                                                                                                                                  | Data availability requirements posted by the optimistic and ZK rollups is precisely the same.                                                                                            | Built-in data availability with validators ensuring distributed state storage and reconstruction in case of disputes.                                                                                                                                                                                                                               |
...
execute computations, manage storage, and interact with smart contracts. Rollups process their own
opcodes, which are interpreted by the EVM during dispute resolution and other operations.
...
<sup>2</sup>There are efforts on allowing native interoperability within rollup hubs, e.g. rollups
within the OP Stack ecosystem will have "better" interoperability than independent L2s.

<sup>3</sup>An escape hatch is method by which users of a rollup can recover digital assets or
...
| **Use Cases**               | Native token transfers, contract execution (though Polkadot allows smart contract execution through XCM, it is almost impossible to use trustlessly in practice, as only very simple calls can be passed without read access), shared staking pools. | Bridging assets and state between rollups.                                              | Cross-chain DeFi, gaming, enterprise blockchain communication.                           |
| **Prone to Centralization** | Low; while Polkadot's design enforces decentralization, it allows the decentralization of collators but does not mandate it, as there are chains with teams operating all collators.                                                                 | Optimism does not give a choice, sequencers must be centralized.                        | Medium; depends on the decentralization level of Chainlink's oracle network.             |
