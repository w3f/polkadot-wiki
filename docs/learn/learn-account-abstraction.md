...
The figure below shows the \{\{ polkadot: Polkadot :polkadot }}\{\{ kusama: Kusama :kusama }}'s
origin abstraction. Accounts happen to be just one variant (or corner case) of Substrate's FRAME
possible origins, the `frame_system::RawOrigin::Signed`. OpenGov origins function in a way that, if
