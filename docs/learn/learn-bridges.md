...
Building a trustless bridge can be done through implementation of the following on-chain components
(ordered by suggested methodology):
...
On-chain bridge components are modules (usually pallets or smart contracts) that are deployed on the
chain's runtime. Modules that track the finality of the source chain are required to be deployed on
the target chain, while the modules that deal with cross-chain messaging need to be deployed on
both, source and target chains.
...
lane, where they are guaranteed to be received in the same order they are sent. On Bridge Hub, the
...
Receiving messages on Polkadot from an external, non-parachain blockchain can thus be possible
through a Substrate pallet. The Substrate instance can then be deployed to Polkadot either as a
system-level parachain (native extension to the core Polkadot software) or as a community-operated
parachain.
...
For the standalone chains that will not have a parachain bridging module the Relay Chain
(non-Substrate), it will be necessary to deploy bridge contracts.
...
Ethereum to infer the finality of transactions on Ethereum chain. Running a GRANDPA light client
