...
take a while depending on the database size. To make this process faster, snapshots can be used. Snaphots
are compressed backups of the database directory of Polkadot/Kusama nodes, containing either the whole chain 
(or a pruned version of it, with only the latest 1000 or 256 blocks). Listed below are a few public snapshot
providers for Polkadot and Kusama. 
