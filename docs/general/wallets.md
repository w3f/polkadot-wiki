...
Supports address only accounts, crowdloans, export/import/derive account(s), ledger support, pool staking, parachain accounts, phishing website detection, proxy management, QR account import, solo staking, support different remote nodes, transfer funds, view balances (Crypto/USD), view transaction history, view an address as QR code.
