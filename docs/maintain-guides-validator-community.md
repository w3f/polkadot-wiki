...
additionally include marketing oneself and participating in the greater community. Becoming a known
participant throughout the ecosystem is a great way to attract nominations and solidify longevity
and sustainability as a validator.
...
commission, and so forth. Including the all suggestions from this page are potential content to
include in the site. After setting up a website, a validator should include this website in the
...
Defining a self allocation strategy is also helpful in seeing how efficient a validator's stake can
be utilized.
...
raise that over time. It can be help to elaborate on these plans in the future. For example "_after
x amount of months in the active set with 0% commission, we plan to increase it to 1%_."
...
validator is hosted in AWS, there is a risk of potential outages causes large amounts of nodes to go
offline, causing slashing for unresponsiveness. Nominators may want to choose validators that have
thoroughly diversified the providers they use or the facilities they operate in.
...
result in less blocks produced and less overall rewards. In certain circumstances more powerful
...
lack thereof) can help get a sense of how robust a validator setup is. There's many common actions
...
they have alerts for particular Github activity? Do they use the Debian/RPM packages? Do they use
...
this can be seen a plus from Nominators, as it helps contribute to decentralization.
...
instill confidence in nominators that various scenarios are thought out and planned for.
...
mean setting up dedicated Telegram / Matrix / Discord channels, or hosting a reoccuring call where
anyone can join. Creating environments that are inclusive with direct connections between parties is
going that extra mile to make sure that Nominators know they're in good hands. There's a lot of
updates that can be given, such as nodes being updated to a new version, rewards were paid out,
servers were migrated, new features or tools have been built, or just checking in to say hello.
These kinds of gestures can be much appreciated in putting words and a person behind the name of
someone running a server.
...
There's many different communities to participate in, from validator, developer, and governance
communities, to local communities dedicated to specific regions. For example, one can be pretty
active in the South American communities, and build a camaraderie among those that speak the same
language, or can go to the same meetups in an area. One of the absolute best ways of building trust
is actually meeting people in person.
...
things, an active participation in the direction of the chain is a great signal that a validator is
there for the good of the network. There are many ways different aspects of governance on can
...
the space. Furthermore, it's possible that one might get tips from the treasury if the community
finds something particularly useful.
